...
import MilanesaSelector, { type MilanesaSelection } from "./MilanesaSelector";
...
export default function ProductModal({ product, onClose, onAdd, isAuthenticated, iceCreamFlavors, iceCreamPotes, pizzaFlavors, tartaFlavors, empanadasFlavors }: Props) {
  ...
  const [milanesaNote, setMilanesaNote] = useState<string | null>(null);
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col flex-1 overflow-y-auto">
          ...
          {isEmpanada && !empanadasNote && (
            <EmpanadasSelector pricePerDozen={parseFloat(product.price)} flavors={empanadasFlavors} onConfirm={handleEmpanadasConfirm} />
          )}
          {isEmpanada && empanadasNote && (
            <div className="mx-5 mt-3 bg-brand-50 border border-brand-200 rounded-2xl px-4 py-3 flex items-start justify-between gap-3">
              <div>
          ...
          {isMilanesa && milanesaNote && (
            <div className="mx-5 mt-3 bg-brand-50 border border-brand-200 rounded-2xl px-4 py-3 flex items-start justify-between gap-3">
              <div>
                <p className="text-xs font-semibold text-brand-600 mb-0.5">Tu selección</p>
                <p className="text-sm text-gray-700">{milanesaNote}</p>
              </div>
              <button onClick={() => setMilanesaNote(null)} className="text-xs text-brand-500 font-medium shrink-0 hover:underline">Cambiar</button>
            </div>
          )}
          ...
          {selectionDone && (
            <div className="px-5 py-4 mt-auto border-t border-gray-100 bg-white">
              <div className="flex items-center justify-between gap-4">
                {!isEmpanada && !isPizza && !isTarta && !isPasta && !isMilanesa && (
